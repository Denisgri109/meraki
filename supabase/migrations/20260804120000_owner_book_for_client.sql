-- T01: owner_book_for_client — owner-guarded, pay-at-venue booking creation for owner-added clients.
-- Mirrors book_pilates_session / book_appointment_with_confirmation semantics minus Stripe,
-- client comes from p_client_id param instead of auth.uid().
-- Applied via Supabase MCP apply_migration (name: owner_book_for_client), 2026-08-03.
...
BEGIN
  ...
  -- BEAUTY BRANCH (mirrors book_appointment_with_confirmation, no Stripe; adds overlap guard
  -- the original lacks so owner double-booking is blocked at RPC level)
