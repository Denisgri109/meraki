...
const passwordKey = (email: string) =>
...
const SEED_ACTIONS: SeedAction[] = [
  ...
  {
    id: "service-create",
    label: "Create Master Service",
    description: "Service: 60 min, €50, with description",
    icon: "content-cut",
    category: "Service Management",
    action: "seed_master_service",
    params: {
      duration: 60,
      price: 50,
      name: "[QA] Test Haircut",
      description: "Seeded by test panel",
    },
  },
  {
    id: "service-custom-pricing",
    label: "Service — Custom Pricing",
    description: "Per-service price override (€75)",
    icon: "content-cut",
    category: "Service Management",
    action: "seed_master_service",
    params: {
      duration: 90,
      price: 75,
      name: "[QA] Premium Styling",
      custom_pricing: true,
    },
  },
  {
    id: "service-custom-duration",
    label: "Service — Custom Duration",
    description: "Custom duration override (120 min)",
    icon: "content-cut",
    category: "Service Management",
    action: "seed_master_service",
    params: {
      duration: 120,
      price: 60,
      name: "[QA] Extended Treatment",
      custom_duration: true,
    },
  },
  {
    id: "service-deposit-override",
    label: "Service — Deposit Override",
    description: "Per-service deposit override (30%)",
    icon: "content-cut",
    category: "Service Management",
    action: "seed_master_service",
    params: {
  ...
  },
  {
    ...
    description: "Disabled/toggled-off service",
    icon: "content-cut",
    category: "Service Management",
    action: "seed_master_service",
    params: {
  ...
  // ═══════════════════════════════════════════════════════════════════
  {
    ...
    description: "Enable deposit with 30% global percentage mode",
    icon: "settings",
    category: "Business Settings",
    action: "seed_business_settings",
    params: {
      section: "deposit",
  ...
  {
    id: "settings-deposit-fixed",
    label: "Deposit — Fixed Amount",
    description: "Enable deposit with €20 fixed amount mode",
    icon: "settings",
    category: "Business Settings",
    action: "seed_business_settings",
    params: {
  ...
  },
  {
    ...
    description: "48h timing, 24h timeout, auto-cancel enabled",
    icon: "pending-actions",
    category: "Business Settings",
    action: "seed_business_settings",
    params: {
  ...
  },
  {
    ...
    description: "50% charge, 15 min threshold, 50% grace multiplier",
    icon: "person-off",
    category: "Business Settings",
    action: "seed_business_settings",
    params: {
  ...
  },
  {
    ...
    description: "Custom terms with require-acceptance enabled",
    icon: "description",
    category: "Business Settings",
    action: "seed_business_settings",
    params: {
      section: "terms",
      custom_terms: "[QA] Test Terms & Conditions — acceptance required",
  ...
  },
  {
    ...
    description: "Push + booking reminders + messages toggles",
    icon: "campaign",
    category: "Business Settings",
    action: "seed_business_settings",
    params: {
  ...
  },
  {
    ...
    label: "Aftercare Campaign",
    description: "Active campaign: 14 days, auto-send, {name} placeholder",
    icon: "favorite",
    category: "Business Settings",
    action: "seed_aftercare_campaign",
    params: {
      days_after: 14,
      auto_send: true,
      message: "Hi {name}, hope you loved your visit! Book again soon.",
    },
  },
...
export function TestPanel() {
  ...
  useEffect(() => {
    (async () => {
      ...
      for (const a of TEST_ACCOUNTS) {
        const key = passwordKey(a.email);
        let pw = await SecureStore.getItemAsync(key);
        if (!pw) {
          pw = await AsyncStorage.getItem(key);
          if (pw) {
      ...
      let legacy = await SecureStore.getItemAsync(LEGACY_PASSWORD_KEY);
      if (!legacy) {
        legacy = await AsyncStorage.getItem(LEGACY_PASSWORD_KEY);
        if (legacy) {
          await SecureStore.setItemAsync(LEGACY_PASSWORD_KEY, legacy);
  ...
  const updateSetting = <K extends keyof SeedSettings>(
    key: K,
    ...
    try {
      ...
      if (error) {
        ...
        if (isInvalidCreds) {
          ...
          await SecureStore.deleteItemAsync(passwordKey(targetEmail));
      ...
      if (cached !== pw) {
        await SecureStore.setItemAsync(passwordKey(targetEmail), pw);
        setSavedPasswords((prev) => ({
          ...prev,
  ...
  const handlePasswordSubmit = async () => {
    ...
    await SecureStore.setItemAsync(passwordKey(pendingAccount), pw);
    setSavedPasswords((prev) => ({
      ...prev,
      [pendingAccount.toLowerCase()]: pw,
    }));
    setHasAnySaved(true);
    setShowPasswordPrompt(false);
  ...
  const handleClearPassword = async () => {
    for (const a of TEST_ACCOUNTS) {
      await SecureStore.deleteItemAsync(passwordKey(a.email));
          ...
          <View style={styles.panel}>
            ...
            >
                    ...
                    >
                      <View style={styles.accountRowLeft}>
                        ...
                        <View>
                          ...
                          <Text style={styles.accountEmail2}>
                            {account.email}
                          </Text>
                        </View>
                      </View>
                  ...
                  <View style={styles.fieldRow}>
                    ...
                    <View style={styles.fieldCell}>
                      ...
                      <TextInput
                        ...
                        onChangeText={(t) =>
                          updateSetting("durationMinutes", t)
                        }
              ...
              <Text style={styles.helperText}>
                Inserts via service-role edge function. Configure who the data
                belongs to in Seed Settings above.
              </Text>
              <Text style={styles.helperHighlight}>
                Client: {settings.clientEmail} · Master: {settings.masterEmail}
              </Text>
              {CATEGORIES.map((category) => {
                const items = SEED_ACTIONS.filter(
              ...
              <Text style={styles.helperText}>
                Schedules local notifications mimicking production push payloads
                to test context deep-linking.
              </Text>
              ...
              <View style={styles.categoryBox}>
                ...
                >
                  ...
                  <View style={styles.categoryRight}>
                    ...
                    <MaterialIcons
                      name={
                        notificationsExpanded ? "expand-less" : "expand-more"
                      }
                      size={20}
                      color={colors.textMuted}
                    />
